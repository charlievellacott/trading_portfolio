...
**Pool candidacy:** candidates come from nested pools in `01_data/processing/s2_universe_pools.py` (`S2_POOLS`) via `iter_pool_pairs`, which walks **any** nesting depth and forms pairs **only within a leaf pool**. Same-venue is asserted per leaf, so a pool mixing exchanges raises instead of producing an unalignable pair. The old manual `KEEP_PAIR_IDS` keep-list is **removed**: it was a discretionary selection channel, replaced by deterministic p-value ranking under caps (H-004).
...
| ----- | ---------- | ----- | ---------------------------------------------- | ------------- | --------------- |
| H-001 | 2026-08-10 | Coint | Universes A–F                                  | —             | DECIDED (C shelved; re-run A–F) |
| H-002 | 2026-08-10 | Coint | 1D vs 4H / 1H after costs                      | —             | NOT IMPLEMENTED |
| H-003 | 2026-08-10 | Coint | Cointegration-break flat rule (Part A)         | —             | NOT IMPLEMENTED |
