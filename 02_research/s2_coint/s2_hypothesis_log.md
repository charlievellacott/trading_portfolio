...
| **Notes**              | Entry/exit = traditional z-score. Spread history obeys PIT β Notes. Isolates hedge quality before gates or alternate scores. **Freeze on fold-val Sharpe** (max DD / corr to S1 secondary); ADF / HL / lag-1 autocorr of `spread` are diagnostics only. **Impl:** 2-state KF tracks [\beta_t, \alpha_t] as a joint random walk; spread / returned β,α are from the **prior** \theta_{t\|t-1} (innovation), never the posterior. Shared core: `feature_implementation/kalman.py`. Store: `compute_kalman_hedge_spread` vs `compute_static_hedge_spread`. Chan (2013) \(\delta=10^{-4}\), \(R=10^{-3}\) makes \(Q/R\) large enough that innovations are nearly white — diagnostic-only, not a freeze candidate. Slower literature: QuantStart/O’Mahony \(\delta=10^{-5}\) with \(R=1\); Palomar §15.6 / Feng & Palomar (2016) \(\alpha=10^{-5}\) (basic) / \(10^{-6}\) (momentum). **Arms:** OLS 252 vs Kalman \(\delta\in\{10^{-5},10^{-6},10^{-7}\}\) (`obs_var=1e-3`, `burn_in=30` days, session-scaled on 1H; \(\delta\) is **not** session-scaled on 1D — if 1H is re-opened, \(Q\approx Q_{\mathrm{day}}/6\)). Type `HEDGE_STAR` (`ols`\|`kalman`) and, if Kalman, `KALMAN_DELTA_STAR`. Notebook: `H-003_hedge.ipynb`. |
