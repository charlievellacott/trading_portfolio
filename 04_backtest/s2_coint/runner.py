...
def _allowed_dates_from_mask(panel: pd.DataFrame, date_mask: pd.Series) -> set:
    """Map a fold mask to calendar dates on the *original* panel.

    Boolean masks are aligned to ``panel`` (often a sliced IS frame whose
    index is not 0..n-1). ``_prepare_panel`` then resets the index, so the
    mask must not be applied with ``loc`` on the prepared frame.
    """
